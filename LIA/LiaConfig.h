...
constexpr double MEASUREMENT_DT = 1.5e-3;
...
public:
    ...
    void setHPFrequency(double freq) {
        ...
        hpfCh[0].y.setCutoffFrequency(freq, MEASUREMENT_DT);
        hpfCh[1].x.setCutoffFrequency(freq, MEASUREMENT_DT);
        hpfCh[1].y.setCutoffFrequency(freq, MEASUREMENT_DT);
    }
    void setLPFrequency(double freq) {
        postCfg.lpFreq = static_cast<float>(freq); // 型変換警告対策
        lpfCh[0].x.setCutoffFrequency(freq, MEASUREMENT_DT);
        lpfCh[0].y.setCutoffFrequency(freq, MEASUREMENT_DT);
        lpfCh[1].x.setCutoffFrequency(freq, MEASUREMENT_DT);
        lpfCh[1].y.setCutoffFrequency(freq, MEASUREMENT_DT);
    }
    void reset() {
        awgCfg = AwgCfg();
        postCfg = PostCfg();
