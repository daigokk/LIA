...
inline void ControlWindow::show(void)
{
    ...
    if (ImGui::BeginTabBar("Awg"))
    {
        ...
        if (ImGui::BeginTabItem("W1"))
        {
            ...
            ImGui::BeginDisabled();
            if (ImGui::InputFloat((char*)u8"Į (Deg.)", &(pSettings->awg.ch[0].phase), 1, 1, "%3.0f"))
            {
                fgFlag = true;
            }
            ImGui::EndTabItem();
        ...
        if (ImGui::BeginTabItem("W2"))
        {
            ...
            ImGui::SetNextItemWidth(nextItemWidth);
            if (ImGui::InputFloat((char*)u8"Į (Deg.)", &(pSettings->awg.ch[1].phase), 1, 1, "%3.0f"))
            {
                fgFlag = true;
            }
            ImGui::EndTabItem();
    ...
    if (ImGui::BeginTabBar("Plot window"))
    {
        if (ImGui::BeginTabItem("XY window"))
        {
            ...
            ImGui::InputDouble((char*)u8"Ch1 Į (Deg.)", &(pSettings->post.offset1Phase), 1.0, 10.0, "%3.0f");
            if (!stateCh2) { ImGui::BeginDisabled(); }
            ImGui::SetNextItemWidth(nextItemWidth);
            ImGui::InputDouble((char*)u8"Ch2 Į (Deg.)", &(pSettings->post.offset2Phase), 1.0, 10.0, "%3.0f");
